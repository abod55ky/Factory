...
export const useEmployees = () => {
  ...
  const createMutation = useMutation({
    mutationFn: async (newEmployee: any) => {
      const payload = { ...newEmployee, hourlyRate: Number(newEmployee.hourlyRate) };
      return await apiClient.post("/employees", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["employees"] });
    ...
    onError: (error: any) => {
      ...
      let finalMessage = typeof serverMessage === "string" ? serverMessage : "حدث خطأ غير متوقع";
      if (finalMessage.includes("employeeId must match")) {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(error.response?.data?.message || "فشل حذف الموظف");
    }
  });
