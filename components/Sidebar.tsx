...
export default function Sidebar() {
  ...
  return (
    // تم إزالة padding من الحاوية الخارجية ليصبح الارتفاع كاملاً بدون فراغات
    <div className="h-screen sticky top-0 flex bg-slate-50 font-sans" dir="rtl">
      <aside
        className={`relative h-full bg-white/70 backdrop-blur-2xl border-y border-l border-white/80 shadow-[0_20px_60px_-15px_rgba(0,187,167,0.15)] rounded-l-[2rem] flex flex-col transition-all duration-500 ease-out z-50
      ...
      >
        ...
        <div className="flex items-center gap-4 p-6 mb-4">
          <div className="relative group shrink-0">
            ...
            <div className="relative bg-gradient-to-br from-[#00bba7] to-[#009485] p-2.5 rounded-xl shadow-md border border-white/20">
          ...
          <div className={`overflow-hidden transition-all duration-500 ease-out ${isCollapsed ? 'w-0 opacity-0' : 'w-full opacity-100'}`}>
            <h1 className="text-2xl font-black bg-gradient-to-l from-slate-800 to-slate-500 bg-clip-text text-transparent tracking-tight">BookS</h1>
        ...
        <nav className="flex-1 space-y-2 px-4 pb-4 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          {visibleMenuItems.map((item) => {
            ...
            const isMainActive = (item.href && pathname.startsWith(item.href)) || (hasSubItems && item.subItems?.some(sub => pathname.startsWith(sub.href)));
              ...
              <div key={item.name} className="relative">
                ...
                {hasSubItems ? (
                  <button
                    ...
                    className={`w-full flex items-center justify-between py-2.5 px-4 rounded-xl transition-all duration-300 group relative overflow-hidden
                      ${isMainActive || isOpen ? 'bg-gradient-to-l from-[#00bba7]/10 to-transparent' : 'hover:bg-slate-50/80'}
                    `}
                  >
                  ...
                  <Link
                    ...
                    className={`flex items-center gap-3 py-2.5 px-4 rounded-xl transition-all duration-300 group relative overflow-hidden
                      ${isMainActive ? 'bg-gradient-to-l from-[#00bba7]/10 to-transparent' : 'hover:bg-slate-50/80'}
                    `}
                  >
                    <div className={`p-2 rounded-xl transition-all duration-300 flex items-center justify-center
                      ${isMainActive 
                ...
                {hasSubItems && !isCollapsed && (
                  <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-60 opacity-100 mt-2' : 'max-h-0 opacity-0'}`}>
                    <div className="flex flex-col gap-1 pr-14 pl-2 relative before:absolute before:right-8 before:top-2 before:bottom-2 before:w-[2px] before:bg-slate-100 before:rounded-full">
                        ...
                        return (
                          <Link
                            ...
                            className={`relative text-sm py-2.5 px-4 rounded-xl transition-all duration-300 flex items-center gap-3
                              ${isSubActive ? 'font-bold text-[#00bba7] bg-white shadow-sm ring-1 ring-slate-100' : 'font-medium text-slate-400 hover:text-slate-700 hover:bg-slate-50 hover:translate-x-[-4px]'}
                            `}
                          >
                            <div className={`absolute right-[-23px] w-3 h-3 rounded-full border-2 border-white transition-all duration-300 ${isSubActive ? 'bg-[#E7C873] scale-100' : 'bg-slate-200 scale-0'}`} />
                            {sub.name}
                          </Link>
                        );
                      })}
        ...
        <div className="p-4 mt-auto shrink-0">
          <div className={`relative overflow-hidden bg-gradient-to-br from-[#00bba7] to-[#008f80] rounded-[1.5rem] shadow-xl shadow-[#00bba7]/30 transition-all duration-500 ${isCollapsed ? 'p-3' : 'p-4'}`}>
            ...
            <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'justify-between'} relative z-10`}>
              ...
              >
                <LogOut 
                  size={18} 
                  strokeWidth={2.5}
                  className={`transition-all duration-300 group-hover:scale-110 group-hover:translate-x-[-4px] ${isCollapsed ? 'rotate-180' : ''}`} 
                />
              </button>
            </div>
          </div>
